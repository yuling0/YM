...
namespace XNodeEditor {
    ...
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, XNode.NodeGraph> {
        ...
        public virtual void AddContextMenuItems(GenericMenu menu) {
            ...
            menu.AddItem(new GUIContent("添加节点"), false, () => { AddContextMenuItems(nodeTypes, pos).ShowInPopup(mousePos); });
        ...
        /// <returns></returns>
        public GenericSelector<Type> AddContextMenuItems(Type[] nodeTypes , Vector2 pos)
        {
            GenericSelector<Type> selector = new GenericSelector<Type>("选择节点");

            if (nodeTypes == null)
            {
                nodeTypes = NodeEditorReflection.nodeTypes.OrderBy(type => GetNodeMenuOrder(type)).ToArray();   //根据排序特性，排序节点
            }
            ...
            for (int i = 0; i < nodeTypes.Length; i++)
            {
                ...
                if (NodeEditorUtilities.GetAttrib(type,out disallowAttrib))
                {
                    int typeCount = target.nodes.Count((node) => { return node.GetType() == type; });
                    if (typeCount >= disallowAttrib.max) disallowed = true;
                }

                if (!disallowed)
                {
                    selector.SelectionTree.Add(path, type);
                }
            }
            Debug.Log($"当前坐标为{pos}");
            selector.SelectionConfirmed += (type) =>
            {
                Type t = type.FirstOrDefault();
                if (t == null) return;
                XNode.Node node = CreateNode(type.FirstOrDefault(), pos);
                NodeEditorWindow.current.AutoConnect(node);
            };
            return selector;
        }
