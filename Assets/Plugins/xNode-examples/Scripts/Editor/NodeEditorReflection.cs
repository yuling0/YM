...
namespace XNodeEditor {
    ...
    public static class NodeEditorReflection {
        ...
        public static Type[] GetDerivedTypes(this Type baseType) {
            List<System.Type> types = new List<System.Type>();//装类型的列表
            System.Reflection.Assembly[] assemblies = System.AppDomain.CurrentDomain.GetAssemblies();//获取当前程序域的所有程序集

            //遍历所有程序集
            foreach (Assembly assembly in assemblies) {
                try {
                    types.AddRange(assembly.GetTypes().Where(t => !t.IsAbstract && baseType.IsAssignableFrom(t)).ToArray());    //获取程序集的所有类型，排除abstract类型，并判断是否继承baseType
                } catch (ReflectionTypeLoadException) { }
            }

            return types.ToArray();//返回获取到的baseType的派生类型
        }
        ...
        public static void AddCustomContextMenuItems(this GenericMenu contextMenu, object obj) {
            ...
            if (items.Length != 0) {
                ...
                for (int i = 0; i < items.Length; i++) {
                    ...
                    if (invalidatedEntries.Contains(kvp.Key.menuItem)) {            //禁用
