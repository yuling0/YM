    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                List<PropertyAttribute> propertyAttributes = NodeEditorUtilities.GetCachedPropertyAttribs(port.node.GetType(), property.name);  //这里获取字段是否添加了Unity内置的一些特性（如：Space、Header等）
        ...
        public static void DrawDynamicList(InspectorProperty property, XNode.NodePort.IO portType, XNode.Node.ConnectionType connectionType, XNode.Node.TypeConstraint typeConstraint)
        {
            string fieldName = property.Name;

            XNode.Node node = property.Tree.WeakTargets[0] as XNode.Node;

            OnDynamicListChanged(property, fieldName, node, portType, connectionType, typeConstraint);

            for (int i = 0 ; i < property.Children.Count; i ++)
            {
                string fn = $"{fieldName} {i}";
                var port = node.GetPort(fn);
                if (port == null)
                {
                    Debug.LogError($"未找到名称为{fieldName} {i}的动态端口");
                    return;
                }
                DrawDynamicPortItem(property.Children[i],port);
            }

        }

        private static void OnDynamicListChanged(InspectorProperty property , string fieldName , XNode.Node node , XNode.NodePort.IO portType , XNode.Node.ConnectionType connectionType , XNode.Node.TypeConstraint typeConstraint)
        {
            property.Update();

            //找出此序列的动态端口
            var dic = node.DynamicPorts.Where((port) =>
            {
                var names = port.fieldName.Split(' ');

                return names.Length == 2 && names[0] == fieldName;
            ...
            for (int i = 0; i < property.Children.Count; i++)
            {
                ...
                if (dic.ContainsKey(fn))    //已存在的动态端口
                {
                    dic.Remove(fn);
                }
                else        //需要添加的动态端口
                {
                    if (portType == XNode.NodePort.IO.Input)
                    {
                        node.AddDynamicInput(propertyChildren.ValueEntry.BaseValueType, connectionType, typeConstraint, fn);
                    }
                    else
                    {
                        node.AddDynamicOutput(propertyChildren.ValueEntry.BaseValueType, connectionType, typeConstraint, fn);
                    }
                }
            }
            ...
            {
                node.RemoveDynamicPort(item.Value);
            }

        }

        private static void DrawDynamicPortItem(InspectorProperty property , XNode.NodePort port)
        {
            ...
            if (isShowing)
            {
                ...
                {
                    pos += new Vector2(rect.width + 16, 0);
                }
            }
            else
            {
                ...
                {
                    pos = rect.position + new Vector2(rect.width, 0);
                }
            }
        ...
        public static void DrawTreeNodePort(GUIContent content, XNode.NodePort port)
        {
            if (port == null) return;
            ...
            if (port.IsInput)
            {
                ...
                pos = rect.position - new Vector2(-rect.width / 2 + 10,5f);
            }
            else if(port.IsOutput)
            {
                Rect rect = GUILayoutUtility.GetLastRect();

                pos = rect.position - new Vector2(-rect.width / 2 + 10, -rect.height);
            }
            PortField(pos, port);
        }
...
#region Obsolete
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType = XNode.Node.ConnectionType.Multiple, XNode.Node.TypeConstraint typeConstraint = XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            ...
            if (list == null) {
                SerializedProperty arrayData = serializedObject.FindProperty(fieldName);//找到是动态列表的这个字段的SerializedProperty
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<XNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType, XNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    serializedObject.ApplyModifiedProperties(); //应用属性更改
                };
