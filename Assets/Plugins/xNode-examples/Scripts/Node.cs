...
#region Inputs/Outputs
        ...
        #endregion
#if UNITY_EDITOR
        int _lastRemoveIndex;
        public void OnDynamicPortListChange(InspectorProperty property , CollectionChangeInfo info , object value)
        {
            string parentName = property.Name;
            if (info.ChangeType == CollectionChangeType.RemoveIndex)
            {
                _lastRemoveIndex = info.Index;
            }
            else if (info.ChangeType == CollectionChangeType.Insert)
            {
                //下移
                if (info.Index > _lastRemoveIndex)
                {
                    for (int i = _lastRemoveIndex; i < info.Index; i++)
                    {
                        NodePort curPort = GetPort($"{parentName} {i}");
                        NodePort swapPort = GetPort($"{parentName} {i+1}");
                        curPort.SwapConnections(swapPort);
                    }
                }
                //上移或者原地
                else
