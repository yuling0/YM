...
namespace XNode {
    ...
    public static class NodeDataCache {
        private static PortDataCache portDataCache;     //缓存的是所有Node的Input和Output的端口字段
        ...
        public static void UpdatePorts(Node node, Dictionary<string, NodePort> ports) {
            ...
            Dictionary<string, NodePort> staticPorts = new Dictionary<string, NodePort>();  //该节点类型的静态端口
            ...
            foreach (NodePort port in ports.Values.ToList()) {
                ...
                if (staticPorts.TryGetValue(port.fieldName, out staticPort)) {
                    ...
                    if (port.IsDynamic || port.direction != staticPort.direction || port.connectionType != staticPort.connectionType || port.typeConstraint != staticPort.typeConstraint) {
                        ...
                        if (!port.IsDynamic && port.direction == staticPort.direction) removedPorts.Add(port.fieldName, port.GetConnections()); //如果端口不是动态并且方向未变，将该端口所连接的端口加入队列
            ...
            foreach (NodePort listPort in dynamicListPorts) {
                ...
                //将序列元素的动态端口属性 为 序列的端口属性（说人话：将数组元素产生的动态端口，将端口属性设置为数组Attribute修饰的属性）
        ...
        /// <summary>
        /// 获取Node的所有字段信息（在派生类中，不能获取父类的隐藏字段）
        /// </summary>
