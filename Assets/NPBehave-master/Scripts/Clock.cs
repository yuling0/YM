...
namespace NPBehave
{
    ...
    public class Clock
    {
        ...
        class Timer
        {
            public double scheduledTime = 0f; //预计时间
            public int repeat = 0;  //重复次数
            public bool used = false;   
			public double delay = 0f;   
			public float randomVariance = 0.0f; //随机变量

            public void ScheduleAbsoluteTime(double elapsedTime)    //计算下一次在哪一时间执行
			{
        ...
        public void Update(float deltaTime)
        {
            ...
            foreach (System.Action action in this.addTimers.Keys)       //在timers添加需要计时的委托函数
            {
                if (this.timers.ContainsKey(action))        //如果有相同函数委托加入
                {
                    Assert.AreNotEqual(this.timers[action], this.addTimers[action]);        //断言它们的计时器不同
                    this.timers[action].used = false;       //把旧的设置为未使用，方便回收
                }
                Assert.IsTrue(this.addTimers[action].used); //断言新添加的函数委托的计时器是使用过的
                this.timers[action] = this.addTimers[action];   //覆盖旧的
            }
