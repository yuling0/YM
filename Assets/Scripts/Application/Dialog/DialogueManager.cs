...
public class DialogueManager : SingletonBase<DialogueManager>
{
    ...
    private DialogueArgs currentDialogueArgs;
    ...
    public void StartDialog(DialogDataContainer dialogData)
    {
        ...
        else if (curNode is EndNode)
        {
            ...
            if (currentDialogueArgs != null)
            {
                currentDialogueArgs?.OnCompleted();
                ReferencePool.Instance.Release(currentDialogueArgs);
                currentDialogueArgs = null;
            }
        }
    }
    public void StartDialog(DialogueArgs args)
    {
        currentDialogueArgs = args;
        if (dic.TryGetValue(args.DialogueName, out DialogDataContainer dialogData))
        {
